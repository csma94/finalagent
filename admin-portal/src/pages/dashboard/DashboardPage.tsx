...
const DashboardPage: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl.replace(/\/$/, '')}/analytics/dashboard`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`API request failed with status ${response.status}: ${errorText || 'No error details'}`);
      }
      ...
      const dashboardMetrics: DashboardMetrics = {
        totalUsers: 0, // Not provided by backend
        activeAgents: result.data.totalAgents || 0,
        activeSites: result.data.sitesMonitored || 0,
        activeShifts: result.data.activeShifts || 0,
        reportsToday: 0, // Not provided by backend
        incidentsToday: result.data.incidentsToday || 0,
        systemHealth: {
          database: 'healthy',
          api: 'healthy',
          websocket: 'healthy',
        ...
        recentActivity: [], // Not provided by backend
      };
      ...
      // Set empty state with error indicators
      setMetrics({
        totalUsers: 0,
        activeAgents: 0,
        activeSites: 0,
